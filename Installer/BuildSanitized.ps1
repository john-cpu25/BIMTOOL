...
$SourceBundleDir = Join-Path $ParentDir "RincoNhan.bundle"
$FinalBinDir = Join-Path $CurrentProjectDir "bin"
$MsiName = "RincoNhanInstaller.msi"
...
New-Item -ItemType Directory -Path (Join-Path $TempBuildDir "RincoNhan.bundle") | Out-Null

# Copy Files
Copy-Item "$CurrentProjectDir\*" (Join-Path $TempBuildDir "Installer") -Exclude "bin","obj","build_log.txt" -Recurse
Copy-Item "$SourceBundleDir\*" (Join-Path $TempBuildDir "RincoNhan.bundle") -Recurse
